...
_EP_RE = re.compile(
    r"Ep\s+(\d+).*?"
    r"Train:\s*loss=([0-9.]+)\s*acc=([0-9.]+)%?\s*f1=([0-9.]+)%?.*?"
    r"Val:\s*loss=([0-9.]+)\s*acc=([0-9.]+)%?\s*f1=([0-9.]+)%?",
    re.IGNORECASE
)
...
class RunPanel(BasePanel):
    ...
    def _build_content(self):
        ...
        #Dataset banner
        self._gate_banner=ctk.CTkFrame(self, fg_color="#3A2A1A", corner_radius=8)
        self._gate_label=ctk.CTkLabel(self._gate_banner, text="Dataset not prepared. Go to Data panel and prepare the dataset before running", font=ctk.CTkFont(size=12, weight="bold"), text_color="#EF9F27", anchor="w", wraplength=900)
        self._gate_label.pack(fill="x", padx=14, pady=10)
    ...
    def _build_stage_cell(self, stage_id: str) -> None:
        ...
        lbl=ctk.CTkLabel(cell, text=f"{_SHORT[stage_id]}", image=self._status_icons[StageStatus.IDLE], font=ctk.CTkFont(size=11), text_color=_STATUS_COLORS[StageStatus.IDLE], width=90, anchor="center")
    ...
    def _start(self):
        ...
        if not self.state.dataset_ready:
            self._log.append("[ERROR] Dataset not prepared. Go to Data panel and prepare dataset first", replace=False)
            return
        cfg_path=self.state.abs_config()
        if not cfg_path.exists():
            self._log.append(f"[ERROR] Config not found: {cfg_path}", replace=False)
            return
        enabled=[sid for sid, en in self.state.stage_enabled.items() if en]
        if not enabled:
            self._log.append("[WARN] No stages enabled", replace=False)
            return
        self._log.clear()
        self._chart.clear()
    ...
    def _poll_log(self):
        try:
            while True:
                ...
                self._log.append(line, replace=False)
    ...
    def _parse_metrics(self, line) -> None:
        ...
        if m and self._active_stage:
            ...
            train_f1=float(m.group(4))/100.0
            val_loss=float(m.group(5))
            val_acc=float(m.group(6))/100.0
            val_f1=float(m.group(7))/100.0
            self._chart.add_point(self._active_stage, epoch, train_loss, val_loss, train_acc, val_acc, train_f1, val_f1)
    ...
    def _on_dataset_changed(self, **_) -> None:
        ...
        if ready:
            self._gate_banner.pack_forget()
            self._run_btn.configure(state="normal")
        else:
            self._gate_banner.pack(fill="x", padx=24, pady=(0, 12), before=self._strip)
            self._run_btn.configure(state="disabled")